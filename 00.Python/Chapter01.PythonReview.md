...
- <h4>语句和表达式基本理解</h4>
  ...
  - **表达式 expression**
    ...
    >若计算结果值未赋值给变量，该行执行完后数据对象一般将在内存销毁。 
...
- <b>Python3中哪些内容被称为表达式，哪些是语句？</b>   
  ...
  **简单的区别方法：**  
  ...
  - **示例2：对于函数表达式的深刻理解**
    ...
    >>> a = print(666)  # 表达式print(666)运算，会打印666，随后将运算后结果赋值给a
    666
    >>> b = None        # 创建None对象，返回None，赋值给b
    >>> c = 1 + 2         # 创键1和2对象，运算完返回3，赋值给c
    ```
    `print()`就和创建`None`，运算`1+2`一样，都是表达式运算的过程，最终返回某个结果。
...
> 都指向了绑定的`数值对象[10]`。  
>
> 变量本身不保存任何数值。变量向另一个变量赋值(绑定)，实际过程是：当前变量查找去赋值的变量所对应的`数值对象`，然后将对应`数值对象`与当前变量进行赋值。  
>
