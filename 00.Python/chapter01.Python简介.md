...
```
类.__dict__   # 查看类对象实例的变量绑定关系__dict__属性绑定一个存储此实例自身变量的字典
```

- 常见系统变量  
`__name__`  当前执行模块(python程序文件)的主名称，一般当前主模块为`"__main__"`  
...
#### 1.6.3.2. `id(obj)` 函数查看对象内存地址代号
  ...
  可用于查看对象存储于计算机内存的“地址”，以判断精确判断对象的存储性质。  
...
---
  ...
  - **示例2：** 探索“小整数对象池”在内存中的存储规则:   
    小整数对象池：python程序创建之初，便初始化-5到256的数在内存中，它们随程序运行周期永远存在，不会释放。  
    也就是说，<font color="red">大于或小于边界数字会创建新对象</font>。  
    ```python
...
问题一: 变量b通过变量a绑定的实质，a绑定到\[10\]，b绑定到a，c绑定到b，b和c都指向哪呢？  
> 都指向了绑定的数值对象\[10\]。  
...
> 会，在第二次赋值时，对象1000的引用计数从1变为0，数值对象被销毁。  
...
补充:   
- 可变对象的区别方法为: 在创建的地址不变情况下，判断对象是否有元素可以发生变化
- 字符串、列表、元组被合并称为序列；  
