...
- 函数初始化：
  ...
  - 形参的初始化
    - 当给定缺省值时，首次调用则将对象创建并赋值给形参，后续如果没有定义则一直使用该对象操作。
  - 函数的返回值定义
    - 函数调用语句是一个表达式，一定会有返回值，能赋值给一个变量。
    - 如果没有return语句，函数执行完毕后返回None对象；如果函数需要返回其它的对象需要用到return语句
- 函数的参数传递：
  - 实参的传递方式
    - 位置传参、序列传参、关键字传参、字典关键字接收
  - 形参的接收方式
    - 普通接收、序列接收、字典接收
  - 函数可以传递的内容可以是任意内容，如同赋值一般。函数中可以再传递函数，函数中也可以再调用函数。
- 命名空间
...
- 函数名的命名规则与变量名相同（函数名必须是标识符）
  - 既然函数名是变量，那么函数名同样满足变量的命名规范和特性：
    ...
    - 可以被del
    - ...
- 假设定义函数func，则`func`与`func()`的区别是什么？
  - `函数名`是变量, 它绑定一个函数。若打印该对象返回一个函数对象内存地址描述。
  - `函数名()`是函数的具体调用，若打印则返回执行调用后的返回值。
...
#### 6.2.3.2. 函数的创建过程与缺省形参理解

- Python程序自上而下运行，函数的初始化一般只能执行一次；函数在一开始初始化时，初始形参的变量即绑定一个被创建的对象。
- 若调用函数不再改变（函数重新初始化，或形参绑定的对象重新创建对象重新赋值），绑定的对象将在之后的调用处理中也一直是该对象；假如是可变对象，将一直持续受到影响。 
