...
* 自动化的升级和维护  
  模式识别的过程可以自动完成，自动更新，易于维护，而且更加准确。

* 解决那些算法过于复杂甚至跟本就没有已知算法的问题

* 在机器学习的过程中协助人类获得对事物的洞见  
  帮助人类发现隐藏在数据中的规则――数据挖掘。
...
1. 人工智能
    1. 机器学习
        1. 传统机器学习：基于标准算法，**中小数据量**
        2. 深度学习：基于深度神经网络，**海量数据量**


## 1.5. 机器学习的种类

### 1.5.1. **监督学习、无监督学习、半监督学习、强化学习**
...
**理解：**  
让不同水平的特征数值统一到一个数学水平，便于比较特征之间的影响力度。
...
# 直接使用API
A = sp.scale(L)
print(A)  # [-1.22474487  0.          1.22474487]
print(A.std())  # 1.0

# 根据原理进行计算
B = L - L.mean()
B /= B.std()
print(B)  # [-1.22474487  0.          1.22474487]
print(B.std())  # 1.0
```
...
# 范围缩放后的样本矩阵 = 范围缩放器.fit_transform(原始样本矩阵) 
```
