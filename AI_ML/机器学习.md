...
with open('AI_ML/ml_data/ratings.json', 'r') as f:
...
train = sd.load_files('AI_ML/ml_data/20news', encoding='latin1',
...
import gensim.corpora as gc
doc = []
with open('AI_ML/ml_data/topic.txt', 'r') as f:
...
sample_rate, sigs = wf.read('AI_ML/ml_data/freq.wav')
print(sample_rate)
...
# 22. n_components: 用几个高斯分布函数拟合样本数据
# 23. covariance_type: 相关矩阵的辅对角线进行相关性比较
# 24. n_iter: 最大迭代上限
model = hl.GaussianHMM(n_components=4, covariance_type='diag', n_iter=1000)
model.fit(mfccs)
# 25. 使用模型匹配测试mfcc矩阵的分值
...
import numpy as np
import scipy.io.wavfile as wf
with open('AI_ML/ml_data/12.json', 'r') as f:
...
wf.write('AI_ML/ml_data/music.wav', sample_rate, music)
```
...
import cv2 as cv
# 26. 读取图片并显示
original = cv.imread('AI_ML/ml_data/forest.jpg')
...
cv.waitKey()
# 30. 图像文件保存
cv.imwrite('AI_ML/ml_data/blue.jpg', blue)
cv.imwrite('AI_ML/ml_data/green.jpg', green)
cv.imwrite('AI_ML/ml_data/red.jpg', red)
cv.imwrite('AI_ML/ml_data/cropped.jpg', cropped)
cv.imwrite('AI_ML/ml_data/scaled1.jpg', scaled1)
cv.imwrite('AI_ML/ml_data/scaled2.jpg', scaled2)
```
...
```python
# 31. 索贝尔边缘识别
# 32. cv.CV_64F：卷积运算使用数据类型为64位浮点型（保证微分的精度）
# 33. 1：水平方向索贝尔偏微分
# 34. 0：垂直方向索贝尔偏微分
# 35. ksize：卷积核为5*5的方阵
...
original = cv.imread( 'AI_ML/ml_data/chair.jpg', cv.IMREAD_GRAYSCALE)
cv.imshow('Original', original)
...
original = cv.imread('AI_ML/ml_data/sunrise.jpg')
cv.imshow('Original', original)
gray = cv.cvtColor(original, cv.COLOR_BGR2GRAY)
cv.imshow('Gray', gray)
equalized_gray = cv.equalizeHist(gray)
...
gray = cv.cvtColor(original, cv.COLOR_BGR2GRAY)
# 42. Harris角点检测器
# 43. 边缘水平方向、垂直方向颜色值改变超过阈值7、5时即为边缘
# 44. 边缘线方向改变超过阈值0.04弧度即为一个角点。
...
original = cv.imread('AI_ML/ml_data/box.png')
cv.imshow('Original', original)
gray = cv.cvtColor(original, cv.COLOR_BGR2GRAY)
cv.imshow('Gray', gray)
corners = cv.cornerHarris(gray, 7, 5, 0.04)
...
# 47. drawKeypoints方法可以把所有的特征点绘制在mixture图像中
...
```python
import cv2 as cv
original = cv.imread('AI_ML/ml_data/table.jpg')
cv.imshow('Original', original)
gray = cv.cvtColor(original, cv.COLOR_BGR2GRAY)
cv.imshow('Gray', gray)
star = cv.xfeatures2d.StarDetector_create()
...
# 49. 加载训练集样本数据，训练模型，模型存储
train_objects = search_files('AI_ML/ml_data/objects/training')
train_x, train_y = [], []
...
# 50. 测试模型
test_objects = search_files('AI_ML/ml_data/objects/testing')
test_x, test_y = [], []
...
# 51. 遍历所有测试样本  使用model匹配测试样本查看每个模型的匹配分数
...
```python
import cv2 as cv
# 56. 通过特征描述文件构建哈尔级联人脸识别器
fd = cv.CascadeClassifier('AI_ML/ml_data/haar/face.xml')
# 57. 从一个图像中识别出所有的人脸区域
# 58. 1.3：为最小的人脸尺寸
# 59. 5：最多找5张脸
# 60. 返回：
# 61. faces: 抓取人脸（矩形区域）列表 [(l,t,w,h),(),()..]
...
# 63. 哈尔级联人脸定位器
fd = cv.CascadeClassifier('AI_ML/ml_data/haar/face.xml')
ed = cv.CascadeClassifier('AI_ML/ml_data/haar/eye.xml')
nd = cv.CascadeClassifier('AI_ML/ml_data/haar/nose.xml')
vc = cv.VideoCapture(0)
while True:
    frame = vc.read()[1]
    faces = fd.detectMultiScale(frame, 1.3, 5)
